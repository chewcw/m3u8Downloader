...
function convertTS() {
    ...
    utils.log(`concat ts to ${mp3 ? 'mp3' : 'mp4'}`);
...
function doConvert(index) {
    if (mp4Num === mp4DoneNum) {
        ...
        const outPutMP4 = mp3 ? `${dir}/output${index}.mp3` : `${dir}/output${index}.mp4`;
        const strConcat = toConcat[index].join('|');
        if (strConcat !== '') {
            if (fs.existsSync(outPutMP4)) {
                fs.unlinkSync(outPutMP4);
            }
            const cmd = mp3 ? `ffmpeg -i "concat:${strConcat}" -acodec copy ${outPutMP4}` : `ffmpeg -i "concat:${strConcat}" -acodec copy -vcodec copy -absf aac_adtstoasc ${outPutMP4}`;
            exec(cmd, (error) => {
                if (error) {
...
function concatMP4() {
    const lastMP4 = mp3 ? `${dir}/${filmName}.mp3` : `${dir}/${filmName}.mp4`;
    if (mp4Num > 1) {
        let filelist = '';
        ...
        let filename = mp3 ? 'output0.mp3' : 'output0.mp4';
        fs.rename(path.join(dir, filename), lastMP4, (err) => {
            if (err) {
